<script lang="ts" use:useRunes>
	...
	const tableName = 'expenseTemplates';

	onMount(async () => {
		const res = await fetch('/api/expenses'); // your read endpoint
		data = await res.json();
	...
	function calcTotal(data, totals) {
		data.forEach((row) => {
			if (row.frequencyUnit == 'year'  && row.status == 'Active') {
				totals.year += (row.amountCents / row.frequencyInterval);
			}
			if (row.frequencyUnit == 'month' && row.frequencyInterval == 3 && row.status == 'Active') {
				totals.quarter += row.amountCents;
			}
			if (row.frequencyUnit == 'month' && row.frequencyInterval != 3 && row.status == 'Active') {
				totals.month += (row.amountCents / row.frequencyInterval);
			}
			if (row.frequencyUnit == 'week' && row.status == 'Active') {
...
<h1 class="p-8 text-lg font-semibold">Master expense record</h1>
<section class="pb-8 ">
{#if data.length === 0}
...
<div class="grid grid-cols-1 gap-6 pb-8 px-8 md:grid-cols-2 lg:grid-cols-4">
	...
	<div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-md">
		...
		<div class="grid grid-cols-2 gap-2 text-sm">
			{#each data as row (row.id)}
				{#if row.frequencyUnit == 'month' && row.frequencyInterval == 1 && row.status == 'Active'}
					<div class="truncate">{row.name}</div>
					<div class="text-right font-medium">{formatCurrency(row.amountCents)}</div>
				{/if}
			{/each}
